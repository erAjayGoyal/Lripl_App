...
var AuthenticationContext = (function() {
  ...
  AuthenticationContext.prototype.login = function() {
    ...
    if (this.config.loginHint !== window.sessionStorage.getItem('EmailId')) {
      this.config.loginHint = window.sessionStorage.getItem('EmailId')
    }
    var urlNavigate =
      this._getNavigateUrl('code', null) +
      '&nonce=' +
      encodeURIComponent(this._idTokenNonce) +
      '&login_hint=' +
      encodeURIComponent(this.config.loginHint)
      ...
      try {
        ...
        if (
          ...
          if (that.isAngular) {
            ...
            let authCode = popUpWindowLocation.search
              .split('?code=')[1]
              .split('&state')[0]
            that.handleWindowCallback(authCode)
          }
          window.clearInterval(pollTimer)
  ...
  AuthenticationContext.prototype.registerCallback = function(
    ...
    if (!this._callBackMappedToRenewStates[expectedState]) {
      this._callBackMappedToRenewStates[expectedState] = function(
        ...
        for (
          var i = 0;
          i < self._callBacksMappedToRenewStates[expectedState].length;
          ++i
        ) {
          try {
  ...
  AuthenticationContext.prototype.acquireTokenPopup = function(
    ...
    var urlNavigate = this._urlRemoveQueryStringParameter(
      this._getNavigateUrl('code', resource),
      'prompt'
    )
  ...
  /**
   * Provides logout clean up for AWS Authentication
   * Navigation redirect is provided and configured in AWS Cognito
   */
  AuthenticationContext.prototype.logOutAWS = function() {
    this.clearCache()
    this._user = null
  }
  /**
  ...
  AuthenticationContext.prototype._createUser = function(idToken) {
    ...
    if (parsedJson && parsedJson.hasOwnProperty('aud')) {
      if (parsedJson.aud.toLowerCase() === this.config.clientId.toLowerCase()) {
        ...
        } else if (parsedJson.hasOwnProperty('userId')) {
          user.userName = parsedJson.userId
        }
      } else {
  ...
  AuthenticationContext.prototype.saveTokenFromHash = function(requestInfo) {
    ...
    // Record error
    if (
      requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)
    ) {
      this.infoPii(
        'Error :' +
      ...
      if (requestInfo.stateMatch) {
        ...
        if (
          requestInfo.parameters.hasOwnProperty(this.CONSTANTS.SESSION_STATE)
        ) {
          this._saveItem(
        ...
        if (
          requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)
        ) {
  ...
  AuthenticationContext.prototype.handleWindowCallback = function(hash) {
    ...
    if (
      ...
      self = this._openedWindows[this._openedWindows.length - 1].opener
        ._adalInstance
      isPopup = true
  ...
  AuthenticationContext.prototype._saveItem = function(key, obj, preserve) {
    if (
      ...
      this.info('Set item in local storage', key, obj)
      if (preserve) {
    ...
    this.info('Set item in session storage', key, obj)
